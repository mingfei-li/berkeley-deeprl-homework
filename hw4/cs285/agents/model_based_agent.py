...
class ModelBasedAgent(nn.Module):
    ...
    def update(self, i: int, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        deltas = (next_obs - obs - self.obs_delta_mean) / (self.obs_delta_std + 1e-6)

        preds = self.dynamics_models[i](obs_acs)
        loss = self.loss_fn(preds, deltas)
    ...
    def update_statistics(self, obs: np.ndarray, acs: np.ndarray, next_obs: np.ndarray):
        ...
        deltas = next_obs - obs
        self.obs_delta_mean = torch.mean(deltas, dim=0, keepdim=True)
        self.obs_delta_std = torch.std(deltas, dim=0, keepdim=True)
