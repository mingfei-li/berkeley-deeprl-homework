...
Consider the $N$-step variant of Q-learning described in lecture. We learn $\Qn$ with the following updates:
\begin{align}
...
\def\answer{24} % <--- TODO: insert index of answer(s)
\ifsolutions\solve\thesubsection\fi
\begin{itemize}
...
\textbf{Solution}: Updated target is 

\[
  y_{j,t} \gets \left(\prod_{t'=t}^{t+N-1}\frac{\pi(a_{t'}|s_{t'})}{\pi'(a_{t'}|s_{t'})}\right)\left(\biggl(\;\sum_{t'=t}^{t+N-1} \gamma^{t'-t} r_{j,t'}\biggr)+\gamma^{N} \max _{\mathbf{a}_{j,t+N}} \Q\left(\mathbf{s}_{j,t+N}, \mathbf{a}_{j,t+N}\right)\right) \\
\]

We do need to change it for $N \to \infty$, but not $N=1$.
