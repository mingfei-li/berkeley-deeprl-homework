...
\end{itemize}
\textbf{Solution:} THe 10-sample configuration achieves much higher return than the single-sample one, because the policy gradient has a much lower variance due to 10x more samples.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{q3_1_3_eval_return.png}
    \caption{Eval return for single-sample REINFORCE (purple) v.s. 10-sample REINFORCE (black)}
    \label{fig:q3_1_3_eval_return}
\end{figure}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q3_1_4_half_cheetah.png}
        \caption{Eval return for HalfCheetah-v4 with single-sample REINFORCE (purple) v.s. 10-sample REINFORCE (green) v.s. REPARAMETRIZE (orange)}
        \label{fig:q3_1_4_half_cheetah}
    \end{figure}
    \item Train an agent for the \verb|Humanoid-v4| environment with \verb|humanoid_sac.yaml| and plot results.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q3_1_4_humanoid.png}
        \caption{Eval return for Humanoid-v4}
        \label{fig:q3_1_4_humanoid}
    \end{figure}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Solution:} Double-Q mitigates overestimation bias a little bit which leads to slightly better results, and Clip-Q mitigates the bias a lot better and hence has a much better retunr (600 v.s. 400).
    \end{itemize}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q3_1_5_hopper_eval_return.png}
        \caption{Eval return for Hopper-v4: single-Q (yellow), double-Q (blue), clipped double-Q (green)}
        \label{fig:q3_1_5_hopper_eval_return}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q3_1_5_hopper_q.png}
        \caption{Q-values for Hopper-v4: single-Q (yellow), double-Q (blue), clipped double-Q (green)}
        \label{fig:q3_1_5_hopper_q}
    \end{figure}
    ...
    \begin{itemize}
        \item \textbf{Solution:} Off-policy algorithm (SAC) is much more sample efficient than on-policy algorithm (Policy Gradient) -- achieves about 1.5x return with the same amount of environment steps.
    \end{itemize}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q3_1_5_humanoid_eval_return.png}
        \caption{Eval return for Humanoid-v4: off-policy (pink) v.s. on-policy (green)}
        \label{fig:q3_1_5_humanoid_eval_return}
    \end{figure}
\end{itemize}