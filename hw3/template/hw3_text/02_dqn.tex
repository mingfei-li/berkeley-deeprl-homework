...
\textbf{Your code may not reach high return (200) on Lunar Lander yet; this is okay!} Your returns may go up for a while and then collapse in some or all of the seeds.
    ...
    \begin{itemize}
        \item \textbf{Solution:} The predicted q-values were extremely overestimated in the beginning (more than 10x of max value possible) and the critic loss was extremely large too. This is related to the fact that q-learning systematically overestimates q-values, and a larger learning rate exaggerates the effect because it learns faster from the initial noisy values.
    \end{itemize}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_cartpole_q_value.png}
        \caption{Q-value plot for CartPole-v1 with lr=0.05}
        \label{fig:q2_4_cartpole_q_value}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_cartpole_critic_loss.png}
        \caption{Critic loss plot for CartPole-v1 with lr=0.05}
