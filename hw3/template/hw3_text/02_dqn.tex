...
\begin{itemize}
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_cartpole.png}
        \caption{Eval return plot for CartPole-v1}
        \label{fig:q2_4_cartpole}
    \end{figure}
...
\textbf{Your code may not reach high return (200) on Lunar Lander yet; this is okay!} Your returns may go up for a while and then collapse in some or all of the seeds.
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_lunarlander.png}
        \caption{Eval return plot for LunarLander-v2 across 3 seeds}
        \label{fig:q2_4_lunarlander}
    \end{figure}
    ...
    \begin{itemize}
        \item \textbf{Solution:} The predicted q-values are more overestimated in the beginning than before and the critic loss is extremely large too. This is related to the fact that q-learning systematically overestimates q-values, and a larger learning rate exaggerates the effect because it learns faster from the initial noisy values.
    \end{itemize}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_cartpole_q_value.png}
        \caption{Q-value plot for CartPole-v1 with lr=0.05 (purple) and lr=1e-3 (orange)}
        \label{fig:q2_4_cartpole_q_value}
    \end{figure}
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_4_cartpole_critic_loss.png}
        \caption{Critic loss plot for CartPole-v1 with lr=0.05 (purple) and lr=1e-3 (orange)}
        \label{fig:q2_4_cartpole_critic_loss}
    \end{figure}
\end{itemize}
...
\end{lstlisting}
    ...
    \begin{itemize}
        \item \textbf{Solution:} Double q-learning is able to achieve above-200 returns consistently, whereas only 1 out of 3 runs of vanilla q-learning barely achieved a 200 return. This is because double q-learning is more resilient to initial random noises and hence leanrs a better policy.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_5_lunarlander.png}
        \caption{Comparison between double q-leraning (red) v.s. vanilla q-learning (blue)}
        \label{fig:q2_5_lunarlander}
    \end{figure}
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Solution:} Initially (step $<$ 30k), the model has just started learning and has such a low quality that it performs worse than taking random actions. Since training uses a much higher exploration rate than eval, it makes more random actions than eval and hence has a higher return. Then, when model quality becomes better, eval has higher returns because it uses better policy from the model more often than training.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_5_mspacman.png}
        \caption{Eval return (pink) and train return (yellow) for MsPacman-v0}
        \label{fig:q2_5_mspacman.png}
    \end{figure}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{q2_6_eval_return.png}
        \caption{Eval return for different learning rates. A lr that's too big/small won't get to a good enough policy.}
        \label{fig:q2_6_eval_return.png}
    \end{figure}
    \end{itemize}
\end{itemize}
